...
# name: basename_numeric_prefix
# purpose: convert integer in file name, like 0000#_file.ext, to integer value
# input: filename
# output: number from within filename as a float
...
# name: normalize_float32_tensor_0_1
# purpose: normalize pixel values
# input: h x w x d image tensor
...
# name: fltbatch_plt
# purpose: output all the images in a batch as individual png files
# input: batch of float32 tensors
# output: file-per-tensor pngs
...
def fltbatch_plt(fltBatch, fltBatchSize, baseNum, name, imageOutDir):
    for i in range(fltBatchSize):
        ...
        plt.subplot(1,1,1)
        plt.imshow(np.transpose(fltBatch[i], (1,0,2)), cmap='gray')#, vmin=0, vmax=1)
        plt.title("")
        plt.axis("off")
