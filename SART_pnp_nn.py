...
def generateParsedArgs():
    ...
    parser.add_argument('--sup_params', dest='sup_params', type=float, nargs=3,\
        help='superiorization parameters: k_min, k_step, gamma')
...
#sino_dir ='/mmfs1/gscratch/uwb/CT_images/sinos2024/'
...
#sinos_dir = '/mmfs1/gscratch/uwb/CT_images/sinos2024'
fnames = []
for filename in os.listdir(infile):
    if filename.endswith(".flt"):
        fnames.append(os.path.join(infile, filename))
...
for n in range(len(fnames)):
    ...
    for k in range(1, numits + 1):
        ...
        if (use_sup) and (k >= kmin) and ((k-kmin)%kstep == 0):
            ...
            f = np.expand_dims(f, axis=-1)
            f = tf.reshape(f,[1,512,512,1])
            f_out = model.predict(f)
